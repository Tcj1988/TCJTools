...
@property (nonatomic, assign) TCJRequestSerializerType baseRequestSerializer;
@property (nonatomic, assign) TCJResponseSerializerType baseResponseSerializer;
@property (nonatomic, assign) BOOL consoleLog;

@end

@implementation TCJRequestEngine{
    NSMutableDictionary * _requestDic;
}

+ (instancetype)defaultEngine{
    static TCJRequestEngine *sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[TCJRequestEngine alloc]init];
    });
...
- (NSUInteger)dataTaskWithMethod:(TCJURLRequest *)request
...
#pragma mark - upload
- (NSUInteger)uploadWithRequest:(TCJURLRequest *)request
    ...
    NSURLSessionDataTask *uploadTask = [self POST:[NSString cj_stringUTF8Encoding:request.url] parameters:request.parameters headers:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        
        [request.uploadDatas enumerateObjectsUsingBlock:^(TCJUploadData *obj, NSUInteger idx, BOOL *stop) {
            if (obj.fileData) {
...
- (NSUInteger)downloadWithRequest:(TCJURLRequest *)request resumeData:(NSData *)resumeData savePath:(NSString *)savePath progress:(void (^)(NSProgress *downloadProgress)) downloadProgressBlock completionHandler:(void (^)(NSURLResponse *response, NSURL *filePath, NSError *error))completionHandler{
    [self headersAndTimeConfig:request];
    [self printParameterWithRequest:request];
    
    NSURLRequest *urlRequest = [NSURLRequest requestWithURL:[NSURL URLWithString:[NSString cj_stringUTF8Encoding:request.url]]];
...
//请求参数的格式
- (void)requestSerializerConfig:(TCJURLRequest *)request{
    self.requestSerializer =request.requestSerializer==TCJHTTPRequestSerializer ?[AFHTTPRequestSerializer serializer]:[AFJSONRequestSerializer serializer];
}

//请求头设置
- (void)headersAndTimeConfig:(TCJURLRequest *)request{
...
#pragma mark - 其他配置
- (void)setupBaseConfig:(TCJConfig *)config{
...
- (void)configBaseWithRequest:(TCJURLRequest *)request progressBlock:(TCJRequestProgressBlock)progressBlock successBlock:(TCJRequestSuccessBlock)successBlock failureBlock:(TCJRequestFailureBlock)failureBlock finishedBlock:(TCJRequestFinishedBlock)finishedBlock target:(id<TCJURLRequestDelegate>)target{
    if (successBlock) {
    ...
    if (request.methodType==TCJMethodTypeUpload) {
        request.apiType=TCJRequestTypeRefresh;
        request.keepType=TCJResponseKeepFirst;
    }
    if (request.methodType==TCJMethodTypeDownLoad) {
        request.apiType=TCJRequestTypeRefresh;
...
- (void)printParameterWithRequest:(TCJURLRequest *)request{
    if (request.consoleLog==YES) {
        NSString *requestStr=request.requestSerializer==TCJHTTPRequestSerializer ?@"HTTP":@"JOSN";
        NSString *responseStr=request.responseSerializer==TCJHTTPResponseSerializer ?@"HTTP":@"JOSN";
        NSLog(@"\n------------TCJNetworking------request info------begin------\n-URLAddress-: %@ \n-parameters-:%@ \n-Header-: %@\n-userInfo-: %@\n-timeout-:%.2f\n-requestSerializer-:%@\n-responseSerializer-:%@\n------------TCJNetworking------request info-------end-------",request.url,request.parameters, self.requestSerializer.HTTPRequestHeaders,request.userInfo,self.requestSerializer.timeoutInterval,requestStr,responseStr);
    }
}
